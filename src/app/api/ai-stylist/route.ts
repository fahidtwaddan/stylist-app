...
// ═══════════════════════════════════════════════════════════════════════
// JOB STORE — in-memory, keyed by jobId
// ═══════════════════════════════════════════════════════════════════════
...
// Clean old jobs every 5 minutes
setInterval(() => {
...
// ═══════════════════════════════════════════════════════════════════════
// PIPELINE STEPS (same logic, now updating job.step for live progress)
// ═══════════════════════════════════════════════════════════════════════
...
async function generateOutfit(photoBase64: string, mediaType: string, profile: Record<string, unknown>, occasion: string) {
  const response = await anthropic.messages.create({
    ...
    messages: [{ role: "user", content: [
      ...
      { type: "text", text: `You are an expert personal stylist for the GCC market. Look at this person's photo and create ONE perfect outfit for: ${occasion}.\n\nStyle Profile: ${JSON.stringify(profile)}\n\nCreate an outfit using items findable on Namshi (UAE's biggest fashion store). Use REAL brand names Namshi carries (Mango, H&M, GANT, Superdry, Lacoste, BOSS, Tommy Hilfiger, Calvin Klein, Adidas, Nike, etc).\n\nReturn JSON:\n{"name":"Creative outfit name","description":"Why this outfit is perfect for ${occasion}","items":[{"name":"Specific product name","brand":"Real brand","category":"tops","price":250,"currency":"AED","searchQuery":"exact search query for Namshi"},{"name":"...","brand":"...","category":"bottoms","price":200,"currency":"AED","searchQuery":"..."}],"totalPrice":450}\n\nRULES:\n- Include exactly 1 top/outerwear + 1 bottom\n- Can include 1-2 more items (shoes, accessories)\n- searchQuery must be specific enough to find a real product on Namshi\n- Prices in AED, realistic for the brand\n- Consider this person's body type, skin tone, and color season\n\nReturn ONLY JSON.` },
    ]}],
  });
  const tb = response.content.find((b) => b.type === "text");
  if (!tb || tb.type !== "text") throw new Error("No response");
  return JSON.parse(extractJSON(tb.text));
}

async function aiPickBest(photoBase64: string, mediaType: string, itemDesc: string, candidates: NamshiProduct[], profile: Record<string, unknown>): Promise<number> {
  ...
  try {
    const r = await anthropic.messages.create({
      ...
      messages: [{ role: "user", content: [
        { type: "image", source: { type: "base64", media_type: mediaType as "image/jpeg" | "image/png" | "image/webp" | "image/gif", data: photoBase64 } },
        { type: "text", text: `Pick the best "${itemDesc}" for this person (${(profile as { archetype?: string }).archetype || ""}, ${(profile as { colorSeason?: string }).colorSeason || ""}):\n${list}\nReply JSON: {"pick": 0}` },
      ]}],
    });
    const tb = r.content.find((b) => b.type === "text");
    if (!tb || tb.type !== "text") return 0;
    const res = JSON.parse(extractJSON(tb.text));
    return Math.min(typeof res.pick === "number" ? res.pick : 0, candidates.length - 1);
  } catch { return 0; }
}
...
// ═══════════════════════════════════════════════════════════════════════
// BACKGROUND PIPELINE — runs async, updates job state
// ═══════════════════════════════════════════════════════════════════════
async function runPipeline(jobId: string, photoBase64: string, mediaType: string, profile: Record<string, unknown>, occasion: string) {
  ...
  try {
    ...
    job.step = "Designing your outfit...";
    console.log(`[Job ${jobId}] Step 1: Generating outfit for "${occasion}"...`);
    const outfit = await generateOutfit(photoBase64, mt, profile, occasion);
    console.log(`[Job ${jobId}] Outfit: ${outfit.name}`);

    // Step 2: Search Namshi
    job.step = "Searching Namshi for real products...";
    ...
    const runFashn = async (): Promise<string | null> => {
      ...
      if (topMatch?.url) {
        ...
        if (r) {
          if (bottomMatch?.url) {
            job.step = "Downloading intermediate result...";
    ...
    job.step = "Generating try-on & analyzing style...";
...
// POST — Start a new styling job, returns immediately with jobId
export async function POST(request: NextRequest) {
  try {
    ...
    runPipeline(jobId, photoBase64, mediaType, profile, occasion);
